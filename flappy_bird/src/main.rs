    ...
    for (ai,a) in dynamics.iter().enumerate() {
        for (bi,b) in dynamics.iter().enumerate().skip(ai+1) {
            if rect_touching(a.rect, b.rect) {
                ...
                into.push(Contact{a:ColliderID::Dynamic(ai), b:ColliderID::Dynamic(bi), mtv});
            }
        }
    }
    ...
    for (ai,a) in dynamics.iter().enumerate() {
        for (bi,b) in statics.iter().enumerate() {
            if rect_touching(a.rect, b.rect) {
                ...
                into.push(Contact{a:ColliderID::Dynamic(ai), b:ColliderID::Static(bi), mtv});
            }
        }
    }
}
...
fn restitute(statics: &[Wall], dynamics:&mut [Mobile], contacts:&mut [Contact]) {
    ...
    for contact in contacts {
        ...
        match contact.a {
            ColliderID::Static(_) => {println!("uh");},
            ColliderID::Dynamic(index_a) => {
                match contact.b {
                    ...
                    ColliderID::Static(index_b) => {
                        ...
                        if x_translate != 0 {obj_a.vx = 0;}
                        if y_translate != 0 {obj_a.vy = 0;}
                        if !a_under {y_translate *= -1;}
                        if !a_totheright {x_translate *= -1;}
